...
All input and output files for this project can be accessed and downloaded from our [GitHub repository](https://github.com/blindedResearch/llm_consistency).
...
The CSV file containing the stock news data can be accessed [here](https://github.com/blindedResearch/llm_consistency/blob/master/data/stock_news_data.csv). The `stock_news_df` data frame contains `r scales::comma(nrow(stock_news_df))` rows and `r ncol(stock_news_df)` columns. The names of the columns are: `r final_string`. Furthermore, the sentiment of the articles is stored in the `sentiment` column. The sentiment of those articles is divided into two categories: `positive`, and `negative`. Their distribution is as follows:
...
The resulting [binary classification dataset](https://github.com/blindedResearch/llm_consistency/blob/master/data/binary_classification_data.csv) 
...
The analysis dataframe for the binary classification experiment has been created. The dataframe contains the LLM labels for each model and replicate, along with the frequency of the most common label across replicates. The results can be accessed in the [binary_analysis_df CSV file](https://github.com/blindedResearch/llm_consistency/blob/main/results/binary_analysis_df.csv).
...
plots = purrr::imap(chat_models, function(model, idx) {
  color = if (idx %% 2 == 1) "#808080"  else darkred
...
The CSV files containing the reliability metrics for the binary classification experiment can be accessed at [binary_reliability_metrics_means.csv](https://github.com/blindedResearch/llm_consistency/blob/main/binary_reliability_metrics_means.csv), [binary_pa_summary.csv](https://github.com/blindedResearch/llm_consistency/blob/main/results/binary_pa_summary.csv), and [binary_reliability_metrics.csv](https://github.com/blindedResearch/llm_consistency/blob/main/results/binary_reliability_metrics.csv). 
...
## Validity of the Binary Classification Labels (Comparison with "Ground Truth" Impacts) {#validity_bin_gold}
...
### Extracting the Ground Truth Stock Performance
...
The comparison against actual market behavior, depicted in the figure below, contrasts the gold standard results. The dot represents each metric's mean value (from its intra-rater five replicates), and the whiskers length reflect the standard error. 
